...
async def on_command_error(ctx, error):
    print(ctx.user.name + " is spamming commands")
...
async def help(ctx):
    print(ctx.user.name + " called the help command")
...
async def check_roles(ctx):
    print(ctx.user.name + " called the check_roles command")
    await ctx.trigger_typing()
...
async def manage_roles(ctx, *args):
    print(ctx.user.name + " called the manage_roles command")
    ...
    try:
        ...
        if ctx.message.author in await reaction.users().flatten():
            ...
            try:
                ...
                print(ctx.user.name + " didn't react in time")
            ...
            async def on_reaction_add(reaction, user):
                print(ctx.user.name + " added this reaction : " + reaction.emoji)
                if user.bot:
                    return
                if reaction.emoji == "❌":
                    print(ctx.user.name + " canceled the manage_roles command")
                ...
                if reaction.emoji == "1️⃣":
                    ...
                    if role.name in [x.name for x in roles]:
                        print(ctx.user.name + " already posses this role: " + role.name)
                        await ctx.send(f"{user.display_name} already posses this role.")
                    else:
                        print(ctx.user.name + " added this role: " + role.name)
            ...
            async def on_reaction_remove(reaction, user):
                ...
                if reaction.emoji == "1️⃣":
                    ...
                    print(ctx.user.name + " removed this role: " + role.name)
